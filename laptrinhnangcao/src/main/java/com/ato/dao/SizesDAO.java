...
@Repository
public class SizesDAO extends HibernateDAO<Sizes, Long> {
    @Autowired
    private SessionFactory sessionFactory;
    public List<SizesDTO> getAllAction(SizesDTO obj){
        Session session = sessionFactory.openSession();
        StringBuilder sql = new StringBuilder("SELECT id, name, code, description, status,update_time updateTime FROM sizes " +
                "  where 1 = 1 ");
        ...
        query.setResultTransformer(Transformers.aliasToBean(SizesDTO.class));
        if (StringUtils.isNotEmpty(obj.getName())) {
