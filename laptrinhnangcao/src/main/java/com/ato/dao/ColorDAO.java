...
@Repository
public class ColorDAO extends HibernateDAO<Color, Long> {
    @Autowired
    private SessionFactory sessionFactory;
    public List<ColorDTO> getAllAction(ColorDTO obj){
        Session session = sessionFactory.openSession();
        StringBuilder sql = new StringBuilder("SELECT id, name, code, description, status,update_time updateTime FROM color " +
                "  where 1 = 1 ");
        ...
        query.setResultTransformer(Transformers.aliasToBean(ColorDTO.class));
        if (StringUtils.isNotEmpty(obj.getName())) {
